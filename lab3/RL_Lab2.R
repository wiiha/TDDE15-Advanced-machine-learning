...
#####################################################################################################
conda_list()
use_condaenv("py3.6-de15-lab3", required = TRUE)
...
vis_prob <- function(goal, episodes = 0){
  ...
  plotObj <- ggplot(df,aes(x = y,y = x)) +
          ...
          ggtitle(paste("Action probabilities after ",episodes," episodes.", "Goal was:",goal[1],",",goal[2])) +
          theme(plot.title = element_text(hjust = 0.5)) +
          scale_x_continuous(breaks = c(1:W),labels = c(1:W)) +
          scale_y_continuous(breaks = c(1:H),labels = c(1:H))
  
  print(plotObj)
  ggsave(paste(print(as.numeric(Sys.time())*1000, digits=15),"-",ranString(1),".png", sep="", collapse=NULL), device = "png")
